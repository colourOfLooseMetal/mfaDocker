...
def extract_segment(mkv_path, start, end, out_path, caption=None, pad_after=0.0):
    ...
    # Build the drawtext filter once if a caption was requested.
    drawtext_filter = None
    if caption:
        caption_path = Path(str(out_path) + ".caption.txt")
...
def build_video(selected_words, index, mkv_dir, output_dir, progress=None,
                burn_subs=BURN_SUBS_DEFAULT, gap=GAP_DEFAULT_SEC):
    """Pick clips, extract, concat. Return the output Path.

    `gap` (seconds) is inserted as held-frame + silence after every word
    except the last.
    """
    ...
    for w in selected_words:
        ...
        stem, start, end = random.choice(occs)
        s = max(0.0, start - PAD_SEC)
        e = end + PAD_SEC
        segments.append((Path(mkv_dir) / f"{stem}.mkv", s, e, w))
    ...
    with tempfile.TemporaryDirectory(prefix="wordvid_") as tmp:
        tmp = Path(tmp)
        for i, (mkv, s, e, word) in enumerate(segments):
            if progress:
                progress(f"extracting {i + 1}/{len(segments)}: {word}")
            pad_after = gap if i < len(segments) - 1 else 0.0
            extract_segment(
                mkv, s, e, tmp / f"seg_{i:04d}.mp4",
                caption=word if burn_subs else None,
                pad_after=pad_after,
            )
        ...
        if progress:
            progress(f"concatenating {len(segments)} segment(s)…")
...
class WordPickerApp:
    def __init__(self, root, index, words):
        self.root = root
        self.index = index
        self.words = words
        self.selected = []
        ...
        self.burn_subs_var = tk.BooleanVar(value=BURN_SUBS_DEFAULT)
        ttk.Checkbutton(action, text="Burn subtitles",
                        variable=self.burn_subs_var).pack(side="right")
    ...
    def _add_word(self, word):
        ...
        self.selected_box.insert(tk.END, word)
        self.selected_box.see(tk.END)
        self.entry.delete(0, tk.END)
        self._refresh_suggestions("")
        self._set_status(f"added '{word}' ({len(self.selected)} selected)")
    ...
    def _on_build(self):
        ...
        burn = bool(self.burn_subs_var.get())
        ...
        def worker():
            try:
                out_path = build_video(
                    words_snapshot, self.index, MKV_DIR, OUTPUT_DIR,
...
def main():
    index, words = build_index(JSON_DIR, MKV_DIR)
    if not words:
        print("No words indexed; aborting.", file=sys.stderr)
        sys.exit(1)
    root = tk.Tk()
    WordPickerApp(root, index, words)
    root.mainloop()
